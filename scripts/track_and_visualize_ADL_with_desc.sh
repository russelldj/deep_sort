...
python evaluate_motchallenge.py --mot_dir MOT16/ADL --detection_dir detections --output_dir outputs/ADL/tracks/tracks-${name}-${stock}-conf${conf}-nms${nms_thresh} --min_confidence ${conf} --nms_max_overlap ${nms_thresh} ${stock} #>> ${log_file} 2>&1
