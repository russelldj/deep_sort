...
#CONFIGS##########################################
declare -a CONFS=(".6" ".3" ".1")
declare -a MIN_IOU_OVERLAPS=("0.05" "0.15" "0.4")
declare -a NMS_THRESHS=("0.05" "0.15" "0.4")
declare -a MAX_COSINE_DISTANCES=("0.2" "0.4" "0.6" "1.0")
declare -a USE_UNMATCHED=(".") # "--use_unmatched" 
#log_file=''
####################################################

source activate motmetrics-env
    ...
    for NMS_THRESH in ${NMS_THRESHS}
        ...
        for min_iou_overlap in ${MIN_IOU_OVERLAPS}
            ...
            for max_cosine_distance in ${MAX_COSINE_DISTANCES}
                do 
                #for use_unmatched in ${USE_UNMATCHED}
                #    do
                    echo $conf ${min_iou_overlaps}
                    ...
                    tracks_dir="outputs/${DATASET}/tracks_with_scores/tracks-${NAME}-conf${conf}-nms${NMS_THRESH}-iou${min_iou_overlap}-max_cosine_distance${max_cosine_distance}${use_unmatched}"
                    log_file="" #">> ${NAME}.txt 2>&1"

                    date ${log_file}
                    #echo name: ${NAME} conf:${conf} stock: ${stock} NMS_THRESH: ${nms_thresh} ${log_file}
                    track_line="python ${PDB} evaluate_motchallenge.py --mot_dir ${MOT_DIR} --detection_dir ${DETECTION_DIR} --output_dir ${tracks_dir} --min_confidence ${conf} ${MAX_COSINE_DISTANCE} --nms_max_overlap ${NMS_THRESH} --min_iou_overlap ${min_iou_overlap} ${stock} ${TRACK_CLASS} ${log_file} --track_subset_file ${VISUALIZE_FRAMES_FILE} ${use_unmatched} --dont_display"
                    echo ${track_line}
                    eval ${track_line}
